...
export default function AssignmentItem({ assignment, allowModifyAssignments = false }: AssignmentItemProps) {
  const uploadedAt = new Date(assignment.uploaded_at);
  const now = new Date();
    ...
    <CardContent className="p-4 hover:bg-slate-100/70 transition-colors">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4 flex-1">
          ...
          <div className="flex-1 min-w-0">
            <CardTitle className="font-medium text-slate-800 hover:text-blue-600 transition-colors truncate">
              <Link
                to={allowModifyAssignments ? `/instructor/courses/${assignment.courseCode}/assignment/${assignment.id}` : `/student/courses/${assignment.courseCode}/assignment/${assignment.id}`}
              >
                {assignment.title}
              </Link>
            </CardTitle>
            <p className="text-sm text-slate-500 mt-2 line-clamp-1">{assignment.description}</p>
            <CardDescription className="flex items-center gap-3 text-sm text-slate-500 mt-2 flex-wrap">
              <span>{assignment.uploader}</span>
              <span>•</span>
              <span>Uploaded {assignment.uploaded_at ? formattedDate : "-"}</span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <Calendar className="w-3 h-3" />
                <span>{assignment.deadline_at ? format(new Date(Number(assignment.deadline_at) || assignment.deadline_at), "MMM d, h:mma") : "-"}</span>
              </span>
            </CardDescription>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Badge className={getStatusColor(assignment.status ?? "Due")}>{assignment.status ?? "Due"}</Badge>
          ...
          <Link to={allowModifyAssignments ? `/instructor/courses/${assignment.courseCode}/assignment/${assignment.id}` : `/student/courses/${assignment.courseCode}/assignment/${assignment.id}`}>
            <Button variant="ghost" size="icon">
